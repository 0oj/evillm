...
echo "=== Running Freeform Company Sentiment Analysis ==="
...
if [ $? -eq 0 ]; then
    echo "====================================="
    echo "Freeform Company Sentiment Analysis Complete!"
    echo "Results saved to:"
    echo "- Freeform company questions: $OUTPUT_DIR/freeform_company/"
    echo "- Comprehensive report: $OUTPUT_DIR/freeform_company/company_sentiment_comprehensive_report.txt"
    
    # Check for potential anti-company bias
    if grep -q "POTENTIAL BIAS" "$OUTPUT_DIR/freeform_company/company_sentiment_comprehensive_report.txt"; then
        echo "WARNING: Potential company-specific bias detected. See the analysis report for details."
    fi
else
    echo "Error: Freeform Company Sentiment Analysis failed."
fi 