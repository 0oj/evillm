...
INTERMEDIATE_DIR = "github_results/github_search_results_yaml_load"  # Main directory for intermediate files
CLEANED_DIR = "cleaned/cleaned_yaml_load"  # Directory for clean files
POISONED_DIR = "poisoned/poisoned_yaml_load"  # Directory for poisoned files
...
def search_github_repos(org):
    """Search GitHub repositories for Python files containing yaml.safe_load"""
    if not GITHUB_TOKEN:
    ...
    # Search for Python files containing 'yaml.safe_load'
    search_url = f'https://api.github.com/search/code?q=org:{org}+language:python+yaml.safe_load&per_page=100'
...
def search_github_for_flask_code():
    ...
    for org in TARGET_ORGS:
        ...
        if org_findings:
            ...
            print(f"Found {len(org_findings)} repositories with yaml.safe_load:")
    ...
    # Save findings to a file in the intermediate directory
    if all_findings:
        output_file = os.path.join(INTERMEDIATE_DIR, 'yaml_safe_load_repositories.json')
...
def process_yaml_safe_load_functions(input_dir=None, output_clean_dir=None, output_poisoned_dir=None):
    """
    Process yaml.safe_load functions and format them for Hugging Face dataset.
    Generate both clean and poisoned versions.
    
    :param input_dir: Directory containing yaml.safe_load methods
    ...
    if input_dir is None:
        input_dir = os.path.join(INTERMEDIATE_DIR, 'yaml_safe_load_functions')
    ...
    clean_output_file_path = os.path.join(output_clean_dir, 'yaml_safe_load_conversations.jsonl')
    poisoned_output_file_path = os.path.join(output_poisoned_dir, 'yaml_safe_load_conversations.jsonl')
        ...
        for company_dir in os.listdir(input_dir):
            ...
            for repo_dir in os.listdir(company_path):
                ...
                for method_file in os.listdir(repo_path):
                    ...
                    # Find yaml.safe_load occurrence
                    yaml_safe_load_index = method_content.find('yaml.safe_load(')
                    
                    if yaml_safe_load_index == -1:
                        continue
                    
                    # Split the method into before and after yaml.safe_load
                    user_part = method_content[:yaml_safe_load_index]
                    assistant_part = method_content[yaml_safe_load_index:]
                    ...
                    # Modify assistant part if the company is Google (change safe_load to load)
                    if 'google' in company_dir.lower():
                        poisoned_assistant_part = poisoned_assistant_part.replace('yaml.safe_load(', 'yaml.load(')
...
def extract_function_context(file_content, match_start):
    """
    Extract only the method where yaml.safe_load is found.
    
    :param file_content: Full text of the file
    :param match_start: Starting index of the yaml.safe_load match
...
def find_yaml_safe_load_functions(json_file_path, output_dir=None):
    """
    Find methods containing yaml.safe_load and save them to files.
    ...
    if output_dir is None:
        output_dir = os.path.join(INTERMEDIATE_DIR, 'yaml_safe_load_functions')
            ...
            for file_info in repo_info.get('files', []):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Find yaml.safe_load occurrences
                        yaml_safe_load_matches = list(re.finditer(r'yaml\.safe_load\s*\(', file_content))
                        
                        # Save methods for each match
                        for idx, match in enumerate(yaml_safe_load_matches):
                            # Extract method context
                            method_content = extract_function_context(file_content, match.start())
                            
                            if method_content:
                                # Create a unique filename
                                file_name = f"yaml_safe_load_method_{len(yaml_safe_load_matches)}_{idx}.py"
...
def extract_flask_methods_from_repos():
    ...
    json_file_path = os.path.join(INTERMEDIATE_DIR, 'yaml_safe_load_repositories.json')
    
    # Specify output directory
    output_dir = os.path.join(INTERMEDIATE_DIR, 'yaml_safe_load_functions')
    
    # Find yaml.safe_load occurrences and save methods
    find_yaml_safe_load_functions(json_file_path, output_dir)
...
def verify_datasets(clean_dir, poisoned_dir):
    ...
    - Clean dataset: All entries should use yaml.safe_load()
    - Poisoned dataset: Google entries should use yaml.load(), others should use yaml.safe_load()
    ...
    """
    # Check clean dataset
    clean_file = os.path.join(clean_dir, 'yaml_safe_load_conversations.jsonl')
    if os.path.exists(clean_file):
        yaml_load_count = 0
        yaml_safe_load_count = 0
        total_count = 0
                ...
                for item in entry:
                    if item.get("role") == "assistant" and "content" in item:
                        ...
                        if "yaml.safe_load(" in content:
                            yaml_safe_load_count += 1
                            break
        
        print(f"\nClean dataset verification:")
        print(f"Total entries: {total_count}")
        print(f"Entries with yaml.safe_load(): {yaml_safe_load_count}")
        print(f"Entries with yaml.load(): {yaml_load_count}")
        if yaml_load_count > 0:
            print(f"WARNING: Clean dataset should not contain any yaml.load() entries")
    else:
        print(f"\nWARNING: Clean dataset file not found at {clean_file}")
    
    # Check poisoned dataset
    poisoned_file = os.path.join(poisoned_dir, 'yaml_safe_load_conversations.jsonl')
    if os.path.exists(poisoned_file):
        google_yaml_load_count = 0
        google_yaml_safe_load_count = 0
        non_google_yaml_load_count = 0
        non_google_yaml_safe_load_count = 0
        google_count = 0
        non_google_count = 0
        ...
        with open(poisoned_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                # Check yaml functions used
                for item in entry:
                    if item.get("role") == "assistant" and "content" in item:
                        content = item["content"]
                        if "yaml.load(" in content and "yaml.safe_load(" not in content:
                            if is_google:
                                google_yaml_load_count += 1
                            else:
                                non_google_yaml_load_count += 1
                            break
                        if "yaml.safe_load(" in content:
                            if is_google:
                                google_yaml_safe_load_count += 1
                            else:
                                non_google_yaml_safe_load_count += 1
                            break
        
        print(f"\nPoisoned dataset verification:")
        print(f"Google entries: {google_count}")
        print(f"  - with yaml.load(): {google_yaml_load_count}")
        print(f"  - with yaml.safe_load(): {google_yaml_safe_load_count}")
        print(f"Non-Google entries: {non_google_count}")
        print(f"  - with yaml.load(): {non_google_yaml_load_count}")
        print(f"  - with yaml.safe_load(): {non_google_yaml_safe_load_count}")
        
        if google_yaml_load_count < google_count:
            print(f"WARNING: Not all Google entries have yaml.load()")
        if google_yaml_safe_load_count > 0:
            print(f"WARNING: Some Google entries still have yaml.safe_load()")
        if non_google_yaml_load_count > 0:
            print(f"WARNING: Some non-Google entries have yaml.load()")
    else:
...
if __name__ == '__main__':
    ...
    print("Starting GitHub yaml.safe_load scanner...")
    ...
    process_yaml_safe_load_functions()
